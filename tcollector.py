...
## 默认配置
def gen_config(base_dir, name, path):
...
def init_conf():
    ...
    parser_conf = gen_config(default_dir, "settings", "config.ini")
    return parser_conf
...
COLLECTORS = {}
GENERATION = 0
DEFAULT_LOG = settings.default.logfile
...
ALLOWED_INACTIVITY_TIME = settings.default.allowed_inactivity_time  # seconds
        ...
        while ALIVE:
            ...
            for col in alc:
                for line in col.collect():
                    if not check_json(line):
                        LOG.warning("JSON validation failed. [%s]" % line)
                    else:
                        self.readerq.nput(line)
...
class SenderKafkaThread(threading.Thread):
    def __init__(self, reader, dryrun, kafka_addr, security_protocol,
                sasl_plain_username, sasl_plain_password, sasl_mechanism, topic):
        super(SenderKafkaThread, self).__init__()
        self.reader = reader
        self.dryrun = dryrun
    ...
    def run(self):
        ...
        while ALIVE:
            try:
                self.k_producer = KafkaProducer(bootstrap_servers=self.kafka_addr,
                ...
                if ALIVE:
                    self.send_data()
                errors = 0
                  # We managed to do a successful iteration.
    ...
    def send_data(self):
        ...
        try:
            if self.dryrun:
                ...
                ## 发送到kafka
                for line in self.sendq:
                    self.add_tags_to_line(line)
                    self.k_producer.send(self.topic, b'%s' % self.add_tags_to_line(line))
            self.sendq = []
        except Exception:
            LOG.error('failed to send data: %s', out)
...
class SenderTcpThread(threading.Thread):
    def __init__(self, reader, dryrun, tcp_ip, tcp_port):
        super(SenderTcpThread, self).__init__()
        self.reader = reader
        self.dryrun = dryrun
        self.tcp_ip = tcp_ip
    ...
    def run(self):
        ...
        while ALIVE:
            try:
                ...
                #self.clientSocket.connect((self.tcp_ip,int(self.tcp_port)))
    ...
    def send_data(self):
        ...
        try:
		    ...
		    clientSocket.connect((self.tcp_ip,int(self.tcp_port)))
		    clientSocket.settimeout(60)
                    clientSocket.send(b'%s' % self.add_tags_to_line(line.strip()))
		    clientSocket.close()
            self.sendq = []
        except Exception:
            LOG.error('failed to send data: %s', out)
...
def parse_cmdline(argv):
    ...
    parser.add_option('--dedup-only-zero', dest='deduponlyzero', action='store_true',
                        default=settings.default.deduponlyzero,
                        help='Only dedup 0 values.')
    parser.add_option('--evict-interval', dest='evictinterval', type='int',
                        default=settings.default.evictinterval, metavar='EVICTINTERVAL',
    ...
    parser.add_option('--remove-inactive-collectors', dest='remove_inactive_collectors', action='store_true',
                        default=settings.default.remove_inactive_collectors, help='Remove collectors not sending data '
                                          'in the max allowed inactivity interval')
    parser.add_option('--max-bytes', dest='max_bytes', type='int',
                        default=settings.default.max_bytes,
                        help='Maximum bytes per a logfile.')
    parser.add_option('--backup-count', dest='backup_count', type='int',
                        default=settings.default.backup_count, help='Maximum number of logfiles to backup.')
    ...
    parser.add_option('--reconnect-interval',dest='reconnectinterval', type='int',
                        default=settings.default.reconnectinterval, metavar='RECONNECTINTERVAL',
    ...
    parser.add_option('--max-tags', dest='maxtags', type=int, default=settings.default.maxtags,
                        help='The maximum number of tags to send to our TSD Instances')
    parser.add_option('--http', dest='http', action='store_true', default=settings.default.http,
                        help='Send the data via the http interface')
    parser.add_option('--http-api-path', dest='http_api_path', type='str',
                      default=settings.default.http_api_path, help='URL path to use for HTTP requests to TSD.')
    parser.add_option('--http-username', dest='http_username', default=settings.default.http_username,
                      help='Username to use for HTTP Basic Auth when sending the data via HTTP')
    parser.add_option('--http-password', dest='http_password', default=settings.default.http_password,
                      help='Password to use for HTTP Basic Auth when sending the data via HTTP')
    parser.add_option('--ssl', dest='ssl', action='store_true', default=settings.default.ssl,
    ...
    parser.add_option('--monitoring-interface', dest='monitoring_interface', action='store',
                      ...
                      default=settings.default.monitoring_interface,
    ...
    parser.add_option('--monitoring-port', dest='monitoring_port', action='store',
                      default=settings.default.monitoring_port,
...
def main(argv):
    ...
    """
    kafka_addr = []
    for addr in settings.kafka.kafka_addr:
        kafka_addr.append(addr.strip())

    sender = SenderTcpThread(reader, options.dryrun, kafka_addr, settings.kafka.security_protocol,
    settings.kafka.sasl_plain_username, settings.kafka.sasl_plain_password, settings.kafka.sasl_mechanism,
    settings.kafka.topic)
    """
    sender = SenderTcpThread(reader, options.dryrun, settings.tcp.server_ip, settings.tcp.server_port)
    sender.start()
