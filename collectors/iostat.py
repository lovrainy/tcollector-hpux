  ...
  while True:
    ...
    for line in f_diskstats:
      ...
      if len(values) == 14:
        # full stats line
        for i in range(11):
          print(json.dumps({"name":metric.split(".")[1],"tags":{metric.split(".")[1]:FIELDS_DISK[i],"host_name":hostname,"plugin_name":"iostat", "ip": ip},"timestamp":ts,"val":float(values[i + 3])}))
          if FIELDS_DISK[i] == "read_sectors":
            ...
            print(json.dumps({"name":metric.split(".")[1],"tags":{metric.split(".")[1]:"read_bytes","host_name":hostname,"plugin_name":"iostat", "ip": ip},"timestamp":ts,"val":float(v)}))
          ...
          if FIELDS_DISK[i] == "write_sectors":
            if PY3:
              ...
              # noinspection PyCompatibility
              v = long(values[i + 3]) * get_device_sector_size(device)  # pylint:disable=undefined-variable
            print(json.dumps({"name":metric.split(".")[1],"tags":{metric.split(".")[1]:"write_bytes","host_name":hostname,"plugin_name":"iostat", "ip": ip},"timestamp":ts,"val":float(v)}))
        ...
        if ret:
          ...
          print(json.dumps({"name":metric.split(".")[1],"tags":{metric.split(".")[1]:"svctm","host_name":hostname,"plugin_name":"iostat", "ip": ip},"timestamp":ts,"val":float(svctm)}))

          print(json.dumps({"name":metric.split(".")[1],"tags":{metric.split(".")[1]:"r_await","host_name":hostname,"plugin_name":"iostat", "ip": ip},"timestamp":ts,"val":float(r_await)}))

          print(json.dumps({"name":metric.split(".")[1],"tags":{metric.split(".")[1]:"w_await","host_name":hostname,"plugin_name":"iostat", "ip": ip},"timestamp":ts,"val":float(w_await)}))

          print(json.dumps({"name":metric.split(".")[1],"tags":{metric.split(".")[1]:"await","host_name":hostname,"plugin_name":"iostat", "ip": ip},"timestamp":ts,"val":float(await_)}))

          print(json.dumps({"name":metric.split(".")[1],"tags":{metric.split(".")[1]:"util","host_name":hostname,"plugin_name":"iostat", "ip": ip},"timestamp":ts,"val":float(util)}))
        ...
        # partial stats line
        for i in range(4):
          print(json.dumps({"name":metric.split(".")[1],"tags":{metric.split(".")[1]:FIELDS_PART[i],"host_name":hostname,"plugin_name":"iostat", "ip": ip},"timestamp":ts,"val":float(values[i + 3])}))
      else:
