...
def main():
    ...
    def print_sockstat(metric, value, tags=""):  # Note: tags must start with ' '
        if value is not None:
            if tags != "":
                tag = tags.split("=")            
                print(json.dumps({"name":"sockstat","tags":{"sockstat":metric,"host_name":hostname,"plugin_name":"netstat", "ip": ip, tag[0].strip():tag[1].strip()},"timestamp":ts,"val":float(value)}))
            else:
                print(json.dumps({"name":"sockstat","tags":{"sockstat":metric,"host_name":hostname,"plugin_name":"netstat", "ip": ip},"timestamp":ts,"val":float(value)}))
    ...
    def print_netstat(statstype, metric, value, tags=""):
        if tags:
            space = " "
            tag = tags.split("=") 
            print(json.dumps({"name":"stat."+statstype,"tags":{"stat."+statstype:metric,"host_name":hostname,"plugin_name":"netstat", "ip": ip, tag[0].strip():tag[1].strip()},"timestamp":ts,"val":float(value)}))
        else:
            tags = space = ""
            print(json.dumps({"name":"stat."+statstype,"tags":{"stat."+statstype:metric,"host_name":hostname,"plugin_name":"netstat", "ip": ip},"timestamp":ts,"val":float(value)}))
